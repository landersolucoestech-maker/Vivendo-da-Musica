...
const CourseManagementDialog = ({
  ...
  const startMaterial = (lessonId: string) => {
  ...
  const saveMaterial = async () => {
    if (!courseId || !materialLessonId || !materialDraft.name.trim() || !materialFile) {
      toast({ title: 'Informe o nome e selecione o arquivo do material.', variant: 'destructive' });
      return;
    }
    ...
    try {
      const uploaded = await courseManagementApi.uploadMaterialFile(courseId, materialLessonId, materialFile);
      ...
      setMaterialFile(null);
      await refreshCourse();
      toast({ title: 'Material enviado e vinculado à aula.' });
    } catch (error) {
      if (uploadedPath) await courseManagementApi.removeMaterialFile(uploadedPath);
  ...
  const openMaterial = async (material: LessonMaterialRecord) => {
    try {
      const url = await courseManagementApi.createSignedMaterialUrl(material.file_url);
      window.open(url, '_blank', 'noopener,noreferrer');
    } catch (error) {
      toast({ title: 'Não foi possível liberar o material.', description: error instanceof Error ? error.message : undefined, variant: 'destructive' });
    }
  };
      ...
      <DialogContent className="max-h-[92vh] max-w-5xl overflow-y-auto">
        ...
        <form onSubmit={saveCourse} className="space-y-6">
          <section className="grid gap-4 rounded-lg border border-border p-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2"><Label htmlFor="course-short-description">Descrição curta</Label><Input id="course-short-description" value={form.short_description ?? ''} disabled={readOnly} onChange={(event) => setForm((current) => ({ ...current, short_description: event.target.value }))} /></div>
            <div className="md:col-span-2"><Label htmlFor="course-description">Descrição completa</Label><Textarea id="course-description" value={form.description ?? ''} disabled={readOnly} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} rows={4} /></div>
          </section>

          <section className="grid gap-4 rounded-lg border border-border p-4 md:grid-cols-3">
            <div className="md:col-span-3"><h3 className="font-semibold">Dados comerciais</h3><p className="text-sm text-muted-foreground">Módulos e aulas não possuem preço.</p></div>
            <div><Label htmlFor="course-original-price">Valor original</Label><Input id="course-original-price" inputMode="decimal" value={originalPrice} disabled={readOnly} onChange={(event) => setOriginalPrice(event.target.value)} /></div>
            <div><Label htmlFor="course-discount">Desconto aplicado</Label><Input id="course-discount" inputMode="decimal" value={discount} disabled={readOnly} onChange={(event) => setDiscount(event.target.value)} /></div>
            <div><Label>Valor final</Label><div className="flex h-10 items-center rounded-md border border-input bg-muted px-3 font-semibold">{formatMoney(finalPrice)}</div></div>
            <div><Label htmlFor="course-status">Status</Label><select id="course-status" className="h-10 w-full rounded-md border border-input bg-background px-3" value={form.status} disabled={readOnly} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value as CourseInput['status'] }))}><option value="draft">Rascunho</option><option value="published">Publicado</option><option value="archived">Arquivado</option></select></div>
            <div><Label htmlFor="course-visibility">Visibilidade</Label><select id="course-visibility" className="h-10 w-full rounded-md border border-input bg-background px-3" value={form.visibility} disabled={readOnly} onChange={(event) => setForm((current) => ({ ...current, visibility: event.target.value as CourseInput['visibility'] }))}><option value="private">Privado</option><option value="public">Público</option><option value="unlisted">Não listado</option></select></div>
          </section>

          {!readOnly && <div className="flex justify-end"><Button type="submit" disabled={saving}>{saving ? 'Salvando...' : courseId ? 'Salvar alterações' : 'Criar curso'}</Button></div>}
        </form>
                  ...
                  <div className="mt-4 space-y-3">
                    ...
                    {module.lessons.map((lesson, lessonIndex) => (
                      <div key={lesson.id} className="rounded-md border border-border p-3">
                        <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs text-muted-foreground">Aula {lessonIndex + 1}</p><p className="font-medium">{lesson.title}</p><p className="text-xs text-muted-foreground">/{lesson.slug}{lesson.duration_minutes ? ` · ${lesson.duration_minutes} min` : ''}</p></div>{!readOnly && <div className="flex gap-2"><Button type="button" size="sm" variant="outline" onClick={() => startLesson(module.id, lesson)}><Pencil className="mr-2 h-4 w-4" />Editar</Button><Button type="button" size="sm" variant="outline" onClick={() => startMaterial(lesson.id)}><FilePlus2 className="mr-2 h-4 w-4" />Material</Button><Button type="button" size="sm" variant="destructive" onClick={() => removeLesson(lesson)}><Trash2 className="h-4 w-4" /></Button></div>}</div>
                        ...
                        {!!lesson.lesson_materials.length && <div className="mt-3 space-y-2"><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Materiais adicionais</p>{lesson.lesson_materials.map((material) => <div key={material.id} className="flex items-center justify-between rounded border border-border px-3 py-2 text-sm"><button type="button" className="text-left underline-offset-4 hover:underline" onClick={() => void openMaterial(material)}>{material.name} · {material.material_type}</button>{!readOnly && <Button type="button" size="sm" variant="ghost" onClick={() => removeMaterial(material.id)}><Trash2 className="h-4 w-4" /></Button>}</div>)}</div>}
                      </div>
                    ))}
                  </div>
                </article>
        ...
        <DialogFooter>{readOnly && <Button type="button" onClick={() => setMode('edit')}><Pencil className="mr-2 h-4 w-4" />Editar curso</Button>}<Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Fechar</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
};
