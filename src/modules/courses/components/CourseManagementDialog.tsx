...
type PendingCurriculumDelete =
  | { kind: 'module'; item: CourseModuleRecord }
  | { kind: 'lesson'; item: LessonRecord };
...
const CourseManagementDialog = ({
  ...
  const [pendingDelete, setPendingDelete] = useState<PendingCurriculumDelete | null>(null);
  ...
  const saveModule = async () => {
    ...
    try {
      ...
      toast({
        title: 'Não foi possível salvar o módulo.',
  ...
  const openMaterial = async (material: LessonMaterialRecord) => {
    try {
      ...
      toast({
        title: 'Não foi possível liberar o material.',
        description: error instanceof Error ? error.message : undefined,
        variant: 'destructive',
      });
    }
  ...
  const confirmCurriculumDelete = async () => {
    if (!pendingDelete) return;
    setDeleteBusy(true);
    try {
      if (pendingDelete.kind === 'module') {
        await courseManagementApi.deleteModule(pendingDelete.item.id);
        toast({ title: 'Módulo excluído.' });
      } else {
        await courseManagementApi.deleteLesson(pendingDelete.item.id);
  ...
  return (
    <>
      ...
      <AlertDialog open={!!pendingDelete} onOpenChange={(nextOpen) => !nextOpen && !deleteBusy && setPendingDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{pendingDeleteTitle}</AlertDialogTitle>
            <AlertDialogDescription>{pendingDeleteDescription} Esta ação não pode ser desfeita.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
