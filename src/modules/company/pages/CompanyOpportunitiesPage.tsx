...
import { BriefcaseBusiness, CalendarClock, Coins, Eye, Pencil, Plus, Power, Trash2 } from 'lucide-react';
...
const kindLabel: Record<CompanyOpportunity['kind'], string> = {
  job: 'Vaga',
  collab: 'Colaboração',
  sync: 'Sync/licenciamento',
  grant: 'Edital',
  contest: 'Concurso',
};

const workModeLabel: Record<CompanyOpportunity['workMode'], string> = {
  onsite: 'Presencial',
  hybrid: 'Híbrido',
  remote: 'Remoto',
};
...
const CompanyOpportunitiesPage = () => {
  ...
  const [viewing, setViewing] = useState<CompanyOpportunity | null>(null);
  ...
  const save = async () => {
    ...
    if (!form.id && creditBalance && availableCredits < 1) {
      toast({ title: 'Sem créditos disponíveis', description: 'Adquira um pacote de publicações antes de publicar uma nova oportunidade.', variant: 'destructive' });
      return;
    }
  ...
  const toggleStatus = async (item: CompanyOpportunity) => {
    if (item.status === 'closed' && creditBalance && availableCredits < 1) {
      toast({ title: 'Sem crédito para renovar', description: 'A renovação da oportunidade exige um novo crédito de publicação.', variant: 'destructive' });
      return;
    }
    ...
    <CompanyLayout>
      <header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="vdm-page-description">Crie, visualize e edite oportunidades somente em popups centralizados.</p>
        </div>
        ...
        <Card>
          <CardContent className="p-5">
            <div className="flex items-start justify-between gap-4">
              <div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">Carteira de vagas</p><p className="mt-3 font-display text-4xl font-bold text-white">{availableCredits}</p><p className="mt-1 text-sm text-muted-foreground">créditos disponíveis</p></div>
            ...
            <div className="mt-5 flex items-center gap-2 border-t border-white/8 pt-4 text-xs text-muted-foreground"><CalendarClock className="size-4 text-primary" />Próxima expiração: {formatDate(creditBalance?.nextExpirationAt ?? null)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            <div className="mb-4"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">Pacotes configurados</p><p className="mt-1 text-sm text-muted-foreground">Valores, quantidades e validades são gerenciados pelo Portal do Administrador.</p></div>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{(creditPacks ?? []).map((pack) => <div key={pack.id} className="rounded-xl border border-white/10 bg-white/[0.02] p-4"><p className="font-semibold text-white">{pack.name}</p><p className="mt-2 text-xl font-bold text-primary">{money.format(pack.priceCents / 100)}</p><p className="mt-1 text-xs text-muted-foreground">{pack.creditQuantity} crédito(s) · validade de {pack.validityDays} dias</p></div>)}</div>
          </CardContent>
        </Card>
      </section>

      {(creditEvents ?? []).length > 0 && <div className="mb-6 rounded-xl border border-white/8 bg-white/[0.02] px-4 py-3 text-xs text-muted-foreground">Última movimentação: {creditEvents?.[0]?.reference || creditEvents?.[0]?.type} · saldo após a operação: {creditEvents?.[0]?.balanceAfter}</div>}

      <div className="mb-6 flex flex-wrap gap-2">{([['all', 'Todas'], ['open', 'Ativas'], ['closed', 'Encerradas']] as const).map(([value, label]) => <Button key={value} variant={filter === value ? 'default' : 'outline'} size="sm" onClick={() => setFilter(value)}>{label}</Button>)}</div>
            ...
            <Card key={item.id}>
              <CardContent className="p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="mb-2 flex flex-wrap gap-2"><Badge variant={item.status === 'open' ? 'success' : 'secondary'}>{item.status === 'open' ? 'Ativa' : 'Encerrada'}</Badge><Badge variant="outline">{workModeLabel[item.workMode]}</Badge>{item.renewalCount > 0 && <Badge variant="outline">{item.renewalCount} renovação(ões)</Badge>}</div>
                  ...
                  <div className="rounded-xl border border-primary/20 bg-primary/8 px-4 py-3 text-center"><p className="font-display text-2xl font-bold text-white">{item.applicationCount}</p><p className="text-[11px] uppercase tracking-[0.1em] text-muted-foreground">candidatos</p></div>
                </div>
                <p className="mt-4 line-clamp-2 text-sm leading-6 text-[#d4d4d4]">{item.description}</p>
                <div className="mt-4 flex flex-wrap gap-4 text-xs text-muted-foreground"><span>{salaryLabel(item)}</span><span>Expira em {formatDate(item.postingExpiresAt)}</span></div>
                <div className="mt-5 flex flex-wrap gap-2 border-t border-white/8 pt-4">
                  <Button variant="outline" size="sm" onClick={() => setViewing(item)}><Eye className="size-4" />Visualizar</Button>
                  <Button variant="outline" size="sm" onClick={() => setForm(toForm(item))}><Pencil className="size-4" />Editar</Button>
                  <Button variant="outline" size="sm" onClick={() => void toggleStatus(item)}><Power className="size-4" />{item.status === 'open' ? 'Encerrar' : 'Renovar — 1 crédito'}</Button>
      ...
      <Dialog open={!!form} onOpenChange={(open) => !open && !busy && setForm(null)}>
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
          <DialogHeader><DialogTitle>{form?.id ? 'Editar oportunidade' : 'Nova oportunidade'}</DialogTitle><DialogDescription>{form?.id ? 'Editar os dados não consome um novo crédito.' : 'Ao publicar, um crédito será consumido da carteira da empresa.'}</DialogDescription></DialogHeader>
          {form && <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="space-y-2"><Label htmlFor="opportunity-kind">Categoria</Label><select id="opportunity-kind" className={selectClassName} value={form.kind} onChange={(event) => setForm({ ...form, kind: event.target.value as FormState['kind'] })}>{Object.entries(kindLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
            ...
            <div className="space-y-2"><Label htmlFor="opportunity-mode">Modelo de trabalho</Label><select id="opportunity-mode" className={selectClassName} value={form.workMode} onChange={(event) => setForm({ ...form, workMode: event.target.value as FormState['workMode'] })}>{Object.entries(workModeLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
          ...
          <DialogFooter><Button variant="outline" disabled={busy} onClick={() => setForm(null)}>Cancelar</Button><Button disabled={busy} onClick={() => void save()}>{busy ? 'Salvando...' : form?.id ? 'Salvar alterações' : 'Publicar — 1 crédito'}</Button></DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={!!viewing} onOpenChange={(open) => !open && setViewing(null)}>
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
          <DialogHeader><DialogTitle>Visualizar oportunidade</DialogTitle><DialogDescription>Detalhes completos, regras de candidatura e histórico da publicação.</DialogDescription></DialogHeader>
          {viewing && <div className="space-y-6">
            <div className="grid gap-4 rounded-xl border border-white/10 bg-white/[0.02] p-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div><p className="text-xs text-muted-foreground">Categoria</p><p className="mt-1 text-white">{kindLabel[viewing.kind]}</p></div>
              <div><p className="text-xs text-muted-foreground">Status</p><p className="mt-1 text-white">{viewing.status === 'open' ? 'Ativa' : 'Encerrada'}</p></div>
              <div><p className="text-xs text-muted-foreground">Local e modelo</p><p className="mt-1 text-white">{viewing.location} · {workModeLabel[viewing.workMode]}</p></div>
              <div><p className="text-xs text-muted-foreground">Contratação</p><p className="mt-1 text-white">{viewing.engagementType}</p></div>
              <div><p className="text-xs text-muted-foreground">Faixa de valor</p><p className="mt-1 text-white">{salaryLabel(viewing)}</p></div>
              <div><p className="text-xs text-muted-foreground">Candidatos</p><p className="mt-1 text-white">{viewing.applicationCount}</p></div>
              <div><p className="text-xs text-muted-foreground">Prazo</p><p className="mt-1 text-white">{formatDate(viewing.applicationDeadline)}</p></div>
              <div><p className="text-xs text-muted-foreground">Expiração</p><p className="mt-1 text-white">{formatDate(viewing.postingExpiresAt)}</p></div>
            </div>
            <div><p className="text-xs text-muted-foreground">Descrição</p><p className="mt-2 whitespace-pre-line text-sm leading-6 text-white">{viewing.description}</p></div>
            <div className="grid gap-5 sm:grid-cols-2">
              <div><p className="mb-2 text-xs text-muted-foreground">Requisitos</p>{viewing.requirements.length ? <ul className="list-inside list-disc space-y-1 text-sm text-white">{viewing.requirements.map((item) => <li key={item}>{item}</li>)}</ul> : <p className="text-sm text-muted-foreground">Nenhum requisito informado.</p>}</div>
              <div><p className="mb-2 text-xs text-muted-foreground">Benefícios</p>{viewing.benefits.length ? <ul className="list-inside list-disc space-y-1 text-sm text-white">{viewing.benefits.map((item) => <li key={item}>{item}</li>)}</ul> : <p className="text-sm text-muted-foreground">Nenhum benefício informado.</p>}</div>
            </div>
            <div className="rounded-xl border border-white/10 p-4 text-xs text-muted-foreground">Publicada em {formatDate(viewing.publishedAt)} · criada em {formatDate(viewing.createdAt)} · {viewing.renewalCount} renovação(ões) · ID {viewing.id}</div>
          </div>}
          <DialogFooter><Button variant="outline" onClick={() => setViewing(null)}>Fechar</Button><Button onClick={() => { const item = viewing; setViewing(null); if (item) setForm(toForm(item)); }}><Pencil className="size-4" />Editar oportunidade</Button></DialogFooter>
        </DialogContent>
      </Dialog>

      <AlertDialog open={!!pendingDelete} onOpenChange={(open) => !open && setPendingDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Excluir oportunidade?</AlertDialogTitle><AlertDialogDescription>A oportunidade “{pendingDelete?.title}” e seu histórico serão excluídos permanentemente. O crédito utilizado na publicação não será devolvido.</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter><AlertDialogCancel disabled={busy}>Cancelar</AlertDialogCancel><AlertDialogAction disabled={busy} onClick={(event) => { event.preventDefault(); void confirmDelete(); }}>{busy ? 'Excluindo...' : 'Excluir definitivamente'}</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </CompanyLayout>
  );
