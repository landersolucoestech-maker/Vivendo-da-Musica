...
const BeatLicenseEditor = ({ beatId, license, onChanged }: { beatId: string; license: BeatLicense; onChanged: () => void }) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await beatService.updateBeatLicense(beatId, license.id, {
        name: String(data.get('name')).trim(), priceCents,
        ...(maxCopiesValue ? { maxCopies: Number(maxCopiesValue) } : {}),
        usageRights: lines(data.get('usageRights')), deliverables: lines(data.get('deliverables')), available,
      });
      setOpen(false);
      onChanged();
      toast({ title: 'Licença atualizada' });
    } catch (error) {
      toast({ variant: 'destructive', title: 'Falha ao editar licença', description: error instanceof Error ? error.message : 'Tente novamente.' });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild><Button size="sm" variant="outline"><Settings2 className="size-3.5" />{license.name}: {formatPrice(license.priceCents, license.currency)}{license.available ? '' : ' (indisponível)'}</Button></DialogTrigger>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
        <DialogHeader><DialogTitle>Editar licença</DialogTitle><DialogDescription>{license.type} · valores persistidos no Supabase dev.</DialogDescription></DialogHeader>
        <form className="grid gap-4 sm:grid-cols-2" onSubmit={(event) => void handleSubmit(event)}>
          ...
          <div className="flex justify-end gap-2 sm:col-span-2"><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancelar</Button><Button type="submit" disabled={saving}>{saving ? 'Salvando...' : 'Salvar licença'}</Button></div>
        </form>
      </DialogContent>
    </Dialog>
  );
