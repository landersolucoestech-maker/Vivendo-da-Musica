...
const mapArticle = (row: Row): ContentArticle => ({
  id: row.id,
  slug: row.slug,
  title: row.title,
  category: row.category ?? 'Conteúdo',
  tag: row.tag ?? '',
  level: (row.level ?? 'Iniciante') as ContentArticle['level'],
  readMinutes: row.read_minutes,
  isFeatured: row.is_featured,
  excerpt: row.excerpt,
  body: row.body,
  author: row.author_name_snapshot,
...
export const contentService = {
  async listArticles(): Promise<ContentArticle[]> {
    const { data, error } = await table('cms_documents')
      .select(fields)
      .eq('document_type', 'article')
      .order('published_at', { ascending: false });

    if (error) throw new Error(`Não foi possível carregar os artigos: ${error.message}`);
    return ((data ?? []) as Row[]).map(mapArticle);
  },

  async getArticleBySlug(slug: string): Promise<ContentArticle | undefined> {
    const { data, error } = await table('cms_documents')
      .select(fields)
      .eq('document_type', 'article')
      .eq('slug', slug)
    ...
    return data ? mapArticle(data as Row) : undefined;
  },

  async listRelatedArticles(article: ContentArticle): Promise<ContentArticle[]> {
    const all = await this.listArticles();
    const explicit = new Set(article.relatedSlugs);
    return all
      .filter((item) => item.slug !== article.slug && (explicit.has(item.slug) || item.category === article.category))
      .slice(0, 3);
  },
  ...
  async listFeaturedArticles(limit = 3): Promise<ContentArticle[]> {
    return (await this.listArticles()).filter((article) => article.isFeatured).slice(0, limit);
  },
};
