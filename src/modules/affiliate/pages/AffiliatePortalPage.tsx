...
} from '@/modules/affiliate/services/affiliate.api';
...
const AffiliatePortalPage = () => {
  ...
  const [viewingWithdrawal, setViewingWithdrawal] = useState<AffiliateWithdrawal | null>(null);
    ...
    if (section === 'withdrawals') {
      return <>
        <SectionHeader
          eyebrow="Financeiro"
          title="Saques"
          description="Solicite o repasse em popup e visualize cada solicitação registrada."
          action={<Button onClick={() => setWithdrawalOpen(true)}><Plus className="size-4" />Solicitar saque</Button>}
        />
        <Card className="mb-6 max-w-md">
          <CardHeader><CardTitle className="text-xl">Saldo disponível</CardTitle><CardDescription>Valor liberado para uma nova solicitação.</CardDescription></CardHeader>
          <CardContent><p className="font-display text-3xl font-bold text-white">{formatCurrency(profile.balance_cents)}</p></CardContent>
        </Card>
        <DataTable
          rows={withdrawals}
          rowKey={(item) => item.id}
          emptyLabel="Nenhum saque solicitado."
          columns={[
            { header: 'Valor', cell: (item) => formatCurrency(item.amount_cents) },
            { header: 'Método', cell: (item) => item.payment_method === 'pix' ? 'Pix' : 'Transferência bancária' },
            { header: 'Solicitação', cell: (item) => formatDate(item.requested_at) },
            { header: 'Status', cell: (item) => <Badge variant={item.status === 'paid' ? 'success' : 'warning'}>{statusLabel(item.status)}</Badge> },
            { header: 'Ações', cell: (item) => <Button size="sm" variant="outline" onClick={() => setViewingWithdrawal(item)}><Eye className="size-4" />Visualizar</Button> },
          ]}
        />
      </>;
    }
  ...
  return (
    <AffiliateLayout>
      ...
      <Dialog open={withdrawalOpen} onOpenChange={(open) => !requestingWithdrawal && setWithdrawalOpen(open)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Solicitar saque</DialogTitle><DialogDescription>Saldo disponível: {formatCurrency(profile.balance_cents)}. O pedido seguirá para análise financeira.</DialogDescription></DialogHeader>
          <form id="affiliate-withdrawal-form" onSubmit={(event) => void submitWithdrawal(event)} className="space-y-4">
            <div className="space-y-2"><Label htmlFor="withdrawal-amount">Valor em reais</Label><Input id="withdrawal-amount" inputMode="decimal" value={withdrawalAmount} onChange={(event) => setWithdrawalAmount(event.target.value)} placeholder="0,00" required /></div>
            <div className="space-y-2"><Label htmlFor="withdrawal-method">Forma de pagamento</Label><select id="withdrawal-method" value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value as 'pix' | 'bank_transfer')} className="h-11 w-full rounded-lg border border-white/10 bg-background px-3 text-sm"><option value="pix">Pix</option><option value="bank_transfer">Transferência bancária</option></select></div>
          </form>
          <DialogFooter><Button variant="outline" disabled={requestingWithdrawal} onClick={() => setWithdrawalOpen(false)}>Cancelar</Button><Button form="affiliate-withdrawal-form" type="submit" disabled={requestingWithdrawal}>{requestingWithdrawal ? 'Registrando...' : 'Solicitar saque'}</Button></DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={!!viewingWithdrawal} onOpenChange={(open) => !open && setViewingWithdrawal(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Visualizar saque</DialogTitle><DialogDescription>Detalhes da solicitação de repasse.</DialogDescription></DialogHeader>
          {viewingWithdrawal && <div className="grid gap-4 rounded-xl border border-white/10 bg-white/[0.02] p-4 sm:grid-cols-2"><div><p className="text-xs text-muted-foreground">Valor</p><p className="mt-1 font-semibold text-white">{formatCurrency(viewingWithdrawal.amount_cents)}</p></div><div><p className="text-xs text-muted-foreground">Status</p><p className="mt-1 text-white">{statusLabel(viewingWithdrawal.status)}</p></div><div><p className="text-xs text-muted-foreground">Método</p><p className="mt-1 text-white">{viewingWithdrawal.payment_method === 'pix' ? 'Pix' : 'Transferência bancária'}</p></div><div><p className="text-xs text-muted-foreground">Solicitado em</p><p className="mt-1 text-white">{formatDate(viewingWithdrawal.requested_at)}</p></div><div className="sm:col-span-2"><p className="text-xs text-muted-foreground">Identificador</p><p className="mt-1 break-all font-mono text-xs text-white">{viewingWithdrawal.id}</p></div></div>}
          <DialogFooter><Button variant="outline" onClick={() => setViewingWithdrawal(null)}>Fechar</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </AffiliateLayout>
  );
