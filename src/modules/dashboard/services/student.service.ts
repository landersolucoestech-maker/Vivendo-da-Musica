...
interface SupportFaqRow { id: string; question: string; answer: string }

const categoryMap: Record<NotificationRow['category'], StudentNotification['category']> = {
  course: 'curso', order: 'pedido', community: 'comunidade', system: 'sistema',
};
const themeMap: Record<PreferencesRow['theme'], StudentSettings['theme']> = {
  system: 'Sistema', light: 'Claro', dark: 'Escuro',
};
const planMap: Record<PreferencesRow['subscription_plan'], StudentSettings['subscriptionPlan']> = {
  free: 'Gratuito', premium: 'Premium', enterprise: 'Enterprise',
};

const getUser = async () => {
  const { data, error } = await supabase.auth.getUser();
  if (error || !data.user) throw new Error('Entre na sua conta para acessar esta area.');
  return data.user;
};
...
export const studentService = {
  async listNotifications(): Promise<StudentNotification[]> {
    ...
    const table = supabase.from as unknown as (name: 'student_notifications') => {
      select(columns: string): {
        eq(column: string, value: string): {
          order(column: string, options: { ascending: boolean }): Promise<{ data: NotificationRow[] | null; error: { message: string } | null }>;
        };
      };
    };
    ...
    if (error) throw new Error(`Nao foi possivel carregar as notificacoes: ${error.message}`);
  ...
  async countUnreadNotifications(): Promise<number> {
    ...
    const { count, error } = await table('student_notifications')
      ...
      .eq('user_id', user.id)
      .is('read_at', null);
    if (error) throw new Error(`Nao foi possivel contar as notificacoes: ${error.message}`);
    return count ?? 0;
  },
  ...
  async markNotificationAsRead(id: string): Promise<{ success: true }> {
    ...
    const { error } = await table('student_notifications')
      ...
      .eq('id', id)
      .eq('user_id', user.id);
    if (error) throw new Error(`Nao foi possivel atualizar a notificacao: ${error.message}`);
    return { success: true };
  },
  ...
  async listSupportTickets(): Promise<SupportTicket[]> {
    ...
    const table = supabase.from as unknown as (name: 'support_tickets') => {
      select(columns: string): { order(column: string, options: { ascending: boolean }): Promise<{ data: SupportTicketRow[] | null; error: { message: string } | null }> };
    };
    const { data, error } = await table('support_tickets')
      .select('id, ticket_code, user_id, subject, message, status, priority, created_at')
      .order('created_at', { ascending: false });
    if (error) throw new Error(`Nao foi possivel carregar os tickets: ${error.message}`);

    const rows = data ?? [];
    const profileTable = supabase.from as unknown as (name: 'user_profiles') => {
      select(columns: string): { in(column: string, values: string[]): Promise<{ data: { user_id: string; full_name: string | null }[] | null; error: { message: string } | null }> };
    };
    const userIds = [...new Set(rows.map((row) => row.user_id))];
    const profiles = userIds.length ? await profileTable('user_profiles').select('user_id, full_name').in('user_id', userIds) : { data: [], error: null };
    if (profiles.error) throw new Error(`Nao foi possivel carregar os solicitantes: ${profiles.error.message}`);
    ...
    return rows.map((row) => ({
      ...
      subject: row.subject,
      message: row.message,
      requester: row.user_id === user.id ? names.get(row.user_id) || user.email || 'Você' : names.get(row.user_id) || `Usuário ${row.user_id.slice(0, 8)}`,
  ...
  async listSupportFaq(): Promise<SupportFaqItem[]> {
    ...
    const table = supabase.from as unknown as (name: 'support_faq') => {
      select(columns: string): { order(column: string, options: { ascending: boolean }): Promise<{ data: SupportFaqRow[] | null; error: { message: string } | null }> };
    };
    const { data, error } = await table('support_faq').select('id, question, answer').order('sort_order', { ascending: true });
    if (error) throw new Error(`Nao foi possivel carregar as perguntas frequentes: ${error.message}`);
    return data ?? [];
  },
  ...
  async openSupportTicket(payload: { subject: string; message: string }): Promise<{ success: true }> {
    const user = await getUser();
    const table = supabase.from as unknown as (name: 'support_tickets') => {
      insert(values: { user_id: string; subject: string; message: string }): Promise<{ error: { message: string } | null }>;
    };
    const { error } = await table('support_tickets').insert({ user_id: user.id, subject: payload.subject.trim(), message: payload.message.trim() });
    if (error) throw new Error(`Nao foi possivel abrir o ticket: ${error.message}`);
    return { success: true };
  },
  async listFavorites(): Promise<Favorite[]> {
    ...
    const table = supabase.from as unknown as (name: 'student_favorites') => {
      select(columns: string): {
        eq(column: string, value: string): {
          order(column: string, options: { ascending: boolean }): Promise<{ data: FavoriteRow[] | null; error: { message: string } | null }>;
        };
      };
    };
    const { data, error } = await table('student_favorites')
      ...
      .eq('user_id', user.id)
      .order('created_at', { ascending: false });
    if (error) throw new Error(`Nao foi possivel carregar os favoritos: ${error.message}`);
    return (data ?? []).flatMap((row): Favorite[] => {
      if (row.course_id && row.courses) return [{ id: row.id, targetId: row.course_id, type: 'curso', title: row.courses.title, meta: formatPriceOrFree(row.courses.price_cents, row.courses.currency), href: ROUTES.academyCourse(row.courses.slug) }];
      if (row.beat_id && row.beats) return [{ id: row.id, targetId: row.beat_id, type: 'produto', title: row.beats.title, meta: row.beats.genre, href: ROUTES.marketplaceBeat(row.beats.slug) }];
      if (row.content_id && row.academy_contents) return [{ id: row.id, targetId: row.content_id, type: 'conteudo', title: row.academy_contents.title, meta: row.academy_contents.category ?? 'Conteudo da Academia', href: ROUTES.academyCourse(row.academy_contents.slug) }];
      return [];
    });
  },
  ...
  async addFavorite(type: FavoriteTargetType, targetId: string): Promise<{ success: true }> {
    ...
    if (error && error.code !== '23505') throw new Error(`Nao foi possivel salvar o favorito: ${error.message}`);
    return { success: true };
  },
  ...
  async removeFavorite(id: string): Promise<{ success: true }> {
    ...
    const { error } = await table('student_favorites').delete().eq('id', id).eq('user_id', user.id);
    if (error) throw new Error(`Nao foi possivel remover o favorito: ${error.message}`);
    return { success: true };
  },
  ...
  async getStudentSettings(): Promise<StudentSettings> {
    ...
    const table = supabase.from as unknown as (name: 'student_preferences') => {
      select(columns: string): {
        eq(column: string, value: string): {
          maybeSingle(): Promise<{ data: PreferencesRow | null; error: { message: string } | null }>;
        };
      };
    };
    const { data, error } = await table('student_preferences')
      .select('course_updates, community_activity, marketing_emails, public_profile, show_progress, locale, theme, subscription_plan')
      .eq('user_id', user.id)
      .maybeSingle();
    if (error) throw new Error(`Nao foi possivel carregar as configuracoes: ${error.message}`);
    ...
    return {
      ...
      privacy: { publicProfile: data.public_profile, showProgress: data.show_progress },
      language: data.locale === 'pt-BR' ? 'Português (Brasil)' : data.locale,
      theme: themeMap[data.theme],
      subscriptionPlan: planMap[data.subscription_plan],
    };
  },
  ...
  async saveStudentSettings(settings: StudentSettings): Promise<{ success: true }> {
    ...
    const reverseTheme: Record<StudentSettings['theme'], PreferencesRow['theme']> = {
      Sistema: 'system', Claro: 'light', Escuro: 'dark',
    };
    const table = supabase.from as unknown as (name: 'student_preferences') => {
    ...
    if (error) throw new Error(`Nao foi possivel salvar as configuracoes: ${error.message}`);
    return { success: true };
  },
};
