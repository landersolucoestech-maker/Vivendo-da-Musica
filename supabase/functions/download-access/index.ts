...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!['beat', 'product'].includes(kind) || typeof id !== 'string') {
    ...
    if (kind === 'beat') {
      ...
      const purchase = data.purchase as unknown as {
        id: string; buyer_id: string | null; contract_number: string; status: string; issued_at: string;
        beat: { title: string; is_demo: boolean }; license: { name: string; usage_rights: unknown; deliverables: unknown };
      };
      ...
      const { data: signed, error: signError } = await admin.storage
        ...
        .createSignedUrl(data.storage_path, 300, { download: data.file_label });
      if (signError || !signed?.signedUrl) return json({ error: 'Arquivo indisponível no Storage.' }, 404);
    ...
    const { data: signed, error: signError } = await admin.storage
      ...
      .createSignedUrl(data.storage_path, 300, { download: data.file_name });
    if (signError || !signed?.signedUrl) return json({ error: 'Arquivo indisponível no Storage.' }, 404);
    return json({ url: signed.signedUrl, expiresIn: 300 });
  } catch {
