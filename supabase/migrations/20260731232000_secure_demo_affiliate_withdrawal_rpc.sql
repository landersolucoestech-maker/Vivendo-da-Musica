-- This development RPC belongs to the affiliate portal domain. Historical local
-- rebuilds may not contain that domain, so create it only when all dependencies exist.
do $migration$
begin
  if to_regclass('public.affiliate_profiles') is null
     or to_regclass('public.affiliate_withdrawals') is null then
    return;
  end if;
  ...
  execute 'revoke all on function public.request_demo_affiliate_withdrawal(bigint, text) from public';
  execute 'grant execute on function public.request_demo_affiliate_withdrawal(bigint, text) to anon, authenticated';
end
$migration$;
