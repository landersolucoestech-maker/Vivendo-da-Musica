...
export const createApprovalLedger = ({ audit = null, now = () => Date.now() } = {}) => {
  ...
  const read = (id) => {
    ...
    verifyRecord(approval, `Approval ${id}`);
    if ((approval.status === 'pending' || approval.status === 'approved') && Date.parse(approval.expiresAt) <= now()) {
      const expired = sealRecord({ ...approval, status: 'expired', updatedAt: new Date(now()).toISOString() });
  ...
  return {
    request({ runId, requestedBy, scope, operation, toolId = null, risk = 'medium', ttlMs = 900000, reason }) {
      ...
      const stored = sealRecord({
        id: crypto.randomUUID(), runId, requestedBy, scope, operation, toolId, risk, reason,
        status: 'pending', createdAt, updatedAt: createdAt,
        expiresAt: new Date(now() + ttlMs).toISOString(), approvedBy: null, decisionReason: null, consumedAt: null
      });
    ...
    decide({ approvalId, actor, decision, reason }) {
      ...
      if (['high', 'critical'].includes(approval.risk) && actor === approval.requestedBy) throw new Error('High-risk approvals require an independent approver');
      const next = sealRecord({ ...approval, status: decision, approvedBy: decision === 'approved' ? actor : null, decisionReason: reason, updatedAt: new Date(now()).toISOString() });
    ...
    consume({ approvalId, runId, scope, operation, toolId = null, actor }) {
      ...
      if (approval.runId !== runId || approval.scope !== scope || approval.operation !== operation || approval.toolId !== toolId) throw new Error('Approval does not match requested operation');
      const timestamp = new Date(now()).toISOString();
      const next = sealRecord({ ...approval, status: 'consumed', consumedAt: timestamp, updatedAt: timestamp });
      approvals.set(approvalId, next);
      audit?.append({ type: 'approval.consumed', actor, runId, payload: { approvalId, scope, operation, toolId } });
      return structuredClone(next);
    },

    get(approvalId) { return structuredClone(read(approvalId)); },
    list({ runId = null, status = null } = {}) {
      return [...approvals.values()].map((approval) => read(approval.id)).filter((approval) => !runId || approval.runId === runId).filter((approval) => !status || approval.status === status).map((approval) => structuredClone(approval));
    },
    isFinal(approvalId) { return finalStates.has(read(approvalId).status); }
  };
};
